---
title: "Whitehead Pilot Trial Sample Size Methods"
...
This document develops and validates the sample-size methods
...
3. Optimal pilot trial sample size approach

The final calculations will be implemented as functions in the
`whiteheadpilot` R package.
...
3. obtain the corresponding upper confidence limit for the standard
   deviation,
4. calculate the main-trial sample size using the UCL method, and
...
The UCL and optimized methods use an 80% upper confidence limit.
```{r}
sd <- 1
effect <- 0.50
power <- 0.90
...
optimized_result <- optimized_ucl_sample_size(
  sd = sd,
  effect = effect,
  power = power,
  alpha = alpha,
...
comparison <- data.frame(
  ...
  main_n_per_arm = c(
    ...
    nct_result$main_n_per_arm,
    optimized_result$main_n_per_arm
  )
)
